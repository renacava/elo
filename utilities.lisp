...
(defun string-to-symbol (string)
  ...
  (if (symbolp string)
      string
      (read-from-string (string-replace-char (format nil "~a" string) " " "-")))
  )
...
;; (defmacro new-plist* (plist)
;;   (let* ((plist-orig plist)
;;          (sequence plist)
;;          (plist (extract-property-list plist))
;;          (pairs (group plist 2))
;;          (keys (remove-if-not #'keywordp plist)))
;;     `(let* ,pairs
;;        ,plist-orig
;;        ,(length plist-orig)
;;        (list ,@(loop for key in keys
;;                      append (list key (property-to-symbol key)))))))
...
(defun property-list-override (src dst)
  "Returns the dst property-list with all items from src inserted and/or overwriting items in dst."
...
(defun hash-to-list (hash-table)
  "Returns a list from hash-table"
  (loop for key being the hash-keys of hash-table
        for value being the hash-values of hash-table
        collect (list key value)))

(defun hash-to-plist (hash-table)
  "Returns a plist from hash-table"
  (loop for key being the hash-keys of hash-table
        for value being the hash-values of hash-table
        append (list (to-property key) value)))

;; (defmacro apply-if (form action predicate)
;;   "Applies action to form when predicate, else returns form unaltered."
;;   `(if ,predicate
;;        (apply ,action (enlist ,form))
;;        ,form))

;; (defmacro with-package (package-name &body body)
;;   (let ((prior-package (package-name *package*)))
;;     `(lambda ()
;;        (in-package ,package-name)
;;        ,@body
;;        (in-package ,prior-package))))
